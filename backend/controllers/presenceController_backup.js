...
// @desc    Démarrer le travail pour un employé
...
exports.pauseWork = async (req, res) => {
    try {
        ...
        if (!presence) {
            return res.status(404).json({
                success: false,
                message: 'Aucune session active trouvée pour aujourd\'hui'
            });
        }
...
exports.resumeWork = async (req, res) => {
    try {
        ...
        if (!presence) {
            return res.status(404).json({
                success: false,
                message: 'Aucune session trouvée pour aujourd\'hui'
            });
        }
...
// @route   GET /api/presences/statistics
// @access  Private (Admin, Manager RH)
exports.getStatistics = async (req, res) => {
    try {
        const { startDate, endDate, employe } = req.query;
        ...
        // Employee filter
        if (employe) {
            matchConditions.employe = employe;
        }

        const presences = await Presence.find(matchConditions).populate('employe', 'nom prenom matricule');
        
        // Calculate statistics
        const stats = {
            totalPresences: presences.length,
            totalWorkHours: presences.reduce((sum, p) => sum + (p.actualWorkHours || 0), 0),
            averageWorkHours: 0,
            anomaliesCount: presences.reduce((sum, p) => sum + (p.anomalies ? p.anomalies.length : 0), 0),
            employeeStats: []
        };
        
        if (stats.totalPresences > 0) {
            stats.averageWorkHours = stats.totalWorkHours / stats.totalPresences;
        }
        ...
        presences.forEach(presence => {
            const empId = presence.employe._id.toString();
            ...
            empStats.totalPresences += 1;
            empStats.totalWorkHours += presence.actualWorkHours || 0;
            empStats.anomaliesCount += presence.anomalies ? presence.anomalies.length : 0;
        });
        
        // Calculate averages
        stats.employeeStats = Array.from(employeeMap.values()).map(emp => ({
            ...emp,
            averageWorkHours: emp.totalPresences > 0 ? emp.totalWorkHours / emp.totalPresences : 0
        }));
